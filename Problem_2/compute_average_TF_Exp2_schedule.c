...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        printf("USE LIKE THIS: ./compute_average_TF_Exp2_schedule input.fna average_TF.csv time.csv num_threads\n");
        exit(-1);
    }
    ...
    fclose(timeFile); fclose(inputFile); fclose(outputFile);
    free(TF); free(genes.gene_sequences); free(genes.gene_sizes);
